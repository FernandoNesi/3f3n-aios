-- 3F3N Dashboard AI — Supabase Schema
-- Mesmas RPCs do v1 (reutilizáveis). Rode UMA vez no SQL Editor.
...
--   click       = qualquer outro evento (ex: 'cta_click', 'view_content')

create or replace function public.get_totals(days_back int default 30)
returns table (
  visits bigint, clicks bigint, conversions bigint, conversion_rate_pct numeric
) language sql stable as $$
  select
    count(*) filter (where event_name = 'page_view') as visits,
    count(*) filter (where event_name not in ('page_view','Lead','Purchase','Schedule','conversion')) as clicks,
    count(*) filter (where event_name in ('Lead','Purchase','Schedule','conversion')) as conversions,
    round(
      count(*) filter (where event_name in ('Lead','Purchase','Schedule','conversion'))::numeric
      / nullif(count(*) filter (where event_name = 'page_view'), 0) * 100, 2
...
returns table (
  page_variation text, visits bigint, clicks bigint,
  conversions bigint, conversion_rate_pct numeric
) language sql stable as $$
  select
...
returns table (
  utm_campaign text, utm_source text,
  visits bigint, conversions bigint, conversion_rate_pct numeric
) language sql stable as $$
  select
